...
public class UsuarioController {
    ...
    public ResponseEntity<Usuario> create(@RequestBody Usuario entity) {
        Usuario nuevo = usuarioService.save(entity);
        return ResponseEntity.status(HttpStatus.CREATED).body(nuevo);
    }

    @PutMapping("/edit/{id}")
    public ResponseEntity<Usuario> edit(@PathVariable int id, @RequestBody Usuario entity) {
        try {
            Usuario actualizado = usuarioService.edit(id, entity);
