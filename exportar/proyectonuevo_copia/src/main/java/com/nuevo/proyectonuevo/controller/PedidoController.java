...
public class PedidoController {
    ...
    public ResponseEntity<Pedido> create(@RequestBody Pedido entity) {
        Pedido nuevo = pedidoService.save(entity);
        return ResponseEntity.status(HttpStatus.CREATED).body(nuevo);
    }

    @PutMapping("/edit/{id}")
    public ResponseEntity<Pedido> edit(@PathVariable int id, @RequestBody Pedido entity) {
        try {
            Pedido actualizado = pedidoService.edit(id, entity);
